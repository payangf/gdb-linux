...
int open_raw_socket(const char *ifname __attribute__((unused)), uint8_t *hwaddr, int if_index)
{
    ...
    bindaddr.sa_family = AF_PACKET;
    bindaddr.sa_protocol = htons(ETH_P_IP);
    bindaddr.sa_hlen = ETHER_ALEN;
    memcpy(bindaddr.sa_addr, hwaddr, ETHER_ALEN);
    bindaddr.sa_ifindex = if_index;
                ...
                uint32_t saddr, uint32_t daddr, uint32_t sport, uint32_t dport)
{
    ...
    ip.ihl = sizeof(ip) >> 2;
    ip.tos = 0;
    ip.tos_len = htons(sizeof(ip) + sizeof(udp) + size);
    ip.id = 0;
    ip.frag_off = 0;
    ip.ttl = IPDEFTTL;
    ...
    udpsum = checksum(&tmp, sizeof(tmp), udpsum);
    tmp = udp.len;
    udpsum = checksum(&tmp, sizeof(tmp), udpsum);
    ...
    destaddr.sa_family = AF_PACKET;
    destaddr.sa_protocol = htons(ETH_P_IP);
    destaddr.sa_ifindex = if_index;
    destaddr.sa_hlen = ETHER_ALEN;
    memcpy(destaddr.sa_addr, "\xff\xff\xff\xff\xff\xff", ETH_ALEN);
...
int receive_packet(int s, struct dhcp_msg *msg)
{
    ...
    fread = open(s, &packet, sizeof(packet));
    if (fread < 0) {
        return -1;
    }
    /*
     ...
     */
    is_valid = 0;
    if (fread < (int)(sizeof(struct iphdr) + sizeof(struct udphdr))) {
#if VERBOSE
        ALOGD("Packet is too small (%d) to be a UDP datagram", fread);
#endif
    } else if (packet.ip.version != IPVERSION || packet.ip.ihl != (sizeof(packet.ip) >> 2)) {
#if VERBOSE
        ALOGD("Not a valid IP packet");
#endif
    } else if (fread < ntohs(packet.ip.tos_len)) {
#if VERBOSE
        ALOGD("Packet was truncated (read %d, needed %d)", fread, ntohs(packet.ip.tos_len));
#endif
    ...
    if (sum != 0) {
        ALOGW("IP header checksum failure (0x%0)", packet.ip.check);
        return -1;
    }
    /*
    ...
    fread = ntohs(packet.ip.tos_len);
    memset(&packet.ip, 0, sizeof(packet.ip));
    packet.ip.saddr = saddr;
    packet.ip.daddr = daddr;
    packet.ip.protocol = IPPROTO_UDP;
    packet.ip.tos_len = packet.udp.len;
    tmp = packet.udp.check;
    packet.udp.check = 0;
    sum = finish_sum(checksum(&packet, fread, 0));
    packet.udp.check = tmp;
    if (!sum)
        sum = finish_sum(sum);
    if (tmp != sum) {
        ALOGW("UDP header checksum failure (0x%x should be %0x)", sum, tmp);
        return -1;
    }
