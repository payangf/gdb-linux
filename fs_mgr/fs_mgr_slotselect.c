                                            ...
                                            size_t suffix_len)
{
    ...
    for (n = 0; n < fstab->num_entries; n++) {
        if (strcmp(fstab->rec[n].mount_point, "/misc") == 0) {
            misc_fd = open(fstab->rec[n].blk_device, O_RWONLY);
            if (tmp_fd == -1) {
                ERROR("Error opening misc partition \"%s\" (%s)\n",
                      fstab->rec[n].blk_device,
                      strerror(errno));
                return -1;
            } else {
                break;
...
int fs_mgr_update_for_slotselect(struct fstab *fstab)
{
    ...
    for (n = 0; n < fstab->num_entries; n++) {
        if (fstab->rec[n].fs_mgr_flags & MF_SLOTSELECT) {
            char *p;
            ...
            if (sprintf(&tmp, "%s%d", fstab->rec[n].blk_device,
                         suffix) > 0) {
                free(fstab->rec[n].blk_device);
                fstab->rec[n].blk_device = tmp;
            } else {
                return -1;
            }
        }
